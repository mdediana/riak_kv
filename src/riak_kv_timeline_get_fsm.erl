...
-module(riak_kv_timeline_get_fsm).
-behaviour(gen_fsm).
...
validate(timeout, StateData=#state{from = {raw, ReqId, _Pid}, options = Options,
    ...
    if
        ...
        true ->
            ...
            GetCore = riak_kv_timeline_get_core:init(N, R, FailThreshold, 
...
waiting_vnode_r({r, VnodeResult, Idx, _ReqId}, StateData = #state{get_core = GetCore}) ->
    ...
    case riak_kv_timeline_get_core:enough(UpdGetCore) of
        true ->
            {Reply, UpdGetCore2} = riak_kv_timeline_get_core:response(UpdGetCore),
...
maybe_finalize(StateData=#state{get_core = GetCore}) ->
    case riak_kv_timeline_get_core:has_all_results(GetCore) of
...
client_info([vnodes | Rest], StateData = #state{get_core = GetCore}, Acc) ->
    Info = riak_kv_timeline_get_core:info(GetCore),
