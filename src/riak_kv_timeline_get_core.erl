...
%%
%% -------------------------------------------------------------------
-module(riak_kv_timeline_get_core).
-export([init/7, add_result/3, enough/1, response/1,
...
-spec init(pos_integer(), pos_integer(), pos_integer(), boolean(), boolean(),
           boolean(), {any | latest}) -> getcore().
init(N, R, FailThreshold, NotFoundOk, AllowMult, DeletedVClock, ReqVersion) ->
    #getcore{n = N,
             r = R,
